...
Catalog uploads use a separate `ADMIN_UPLOAD_SIGNATURES_PER_HOUR` allowance, defaulting to 100 (configurable from 1 to 500). Customer orders and inquiry images continue sharing the 20-per-hour customer limit. This allows the configured owner to maintain multi-image products without exhausting the customer allowance.
