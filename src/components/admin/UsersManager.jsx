      ...
      <div className="admin-panel admin-table-panel users-table-panel">
        {loading ? <AdminSectionState loading title="Loading customer accounts" message="Reviewing signups and verification details…"/> : users.length ? <>
          <Table responsive hover className="admin-table users-table"><thead><tr><th>Customer</th><th>Phone</th><th>Joined</th><th>Last sign-in</th><th>Access</th><th><span className="visually-hidden">Open</span></th></tr></thead><tbody>{users.map((account) => {
            ...
            return <tr key={id} className={selected && (selected.id || selected._id) === id ? 'is-selected' : ''}><td><button type="button" className="user-table-identity" onClick={(event) => openUser(account, event.currentTarget)} aria-label={`View details for ${account.name || account.email}`}><b>{account.avatar || account.picture ? <img src={account.avatar || account.picture} alt="" referrerPolicy="no-referrer"/> : initials(account.name, account.email)}</b><span><strong>{account.name || 'Customer'}</strong><small>{account.email}</small></span></button></td><td><span className="user-phone">{account.phone || 'Not added'}{account.phone && <i className={verifiedPhone ? 'is-verified' : ''}>{verifiedPhone ? 'Verified' : 'Pending'}</i>}</span></td><td>{dateLabel(account.createdAt || account.joinedAt)}</td><td>{dateLabel(account.lastLoginAt || account.lastSeenAt)}</td><td><span className={`user-role user-role--${account.role || 'buyer'}`}>{account.role === 'admin' ? 'Admin' : 'Customer'}</span></td><td><button type="button" className="user-open" onClick={(event) => openUser(account, event.currentTarget)} aria-label={`View ${account.name || account.email}`}><Icon name="arrow" size={17}/></button></td></tr>;
          })}</tbody></Table>
...
function UserDetail({ account, loading, error, onClose, panelRef }) {
  ...
  return <aside ref={panelRef} className="user-detail" role="region" tabIndex={-1} aria-labelledby="admin-user-detail-title">
    ...
    <div className="user-detail__identity"><span>{account.avatar || account.picture ? <img src={account.avatar || account.picture} alt="" referrerPolicy="no-referrer"/> : initials(account.name, account.email)}</span><h3 id="admin-user-detail-title">{account.name || 'Customer'}</h3><p>{account.email}</p><b className={`user-role user-role--${account.role || 'buyer'}`}>{account.role === 'admin' ? 'Administrator' : 'Customer'}</b></div>
    ...
    <section><h4>Identity & verification</h4><dl><div><dt>Email</dt><dd>{account.email}<span className={account.emailVerified === false ? 'is-pending' : 'is-verified'}>{account.emailVerified === false ? 'Verification not recorded' : 'Verified email'}</span></dd></div><div><dt>Sign-in</dt><dd>{providers.length?<span className="identity-provider-list">{providers.map((provider)=><b key={provider}>{providerLabels[provider]}</b>)}</span>:<span>Provider details not recorded</span>}</dd></div><div><dt>Phone</dt><dd>{account.phone || 'Not provided'}{account.phone && <span className={verifiedPhone ? 'is-verified' : 'is-pending'}>{verifiedPhone ? 'OTP verified' : 'Verification pending'}</span>}</dd></div></dl></section>
